...
function syncToCalendar() {
  ...
  if (toDelete.length > 0) {
    ...
    if (go === ui.Button.YES) {
      toDelete.forEach(function(ev, i) { if (i > 0 && i % 10 === 0) Utilities.sleep(1000); Calendar.Events.remove(calId, ev.id); deleted++; });
    }
  }
...
function syncEventsToCalendar() {
  ...
  allEvents.forEach(function(ev) {
    var desc = ev.description || '';
    var match = desc.match(/\\[muse_event:[^\\]]+\\]/);
    if (match && !currentTags[match[0]]) { Calendar.Events.remove(calId, ev.id); Utilities.sleep(200); }
  });
}
